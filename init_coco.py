...
if __name__ == "__main__":
    ...
    idx_mapping = {idx1: idx2 for idx1, idx2 in zip(range(133), range(133))}
    inv_idx_mapping = {idx1: idx2 for idx1, idx2 in zip(range(133), range(133))}

    cat_json = json.load(open('panopticapi/panoptic_coco_categories.json'))
    ...
    json.dump(cat_json_stff, open('data/coco/annotations/panoptic_coco_categories_Easystuff.json', 'w'))
    ...
    for s in ['train', 'val']:
        ...
        json.dump(pano_json_stff, open('data/coco/annotations/panoptic_{}2017_Easystuff.json'.format(s), 'w'))
